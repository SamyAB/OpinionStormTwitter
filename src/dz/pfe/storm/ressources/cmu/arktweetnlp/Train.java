...
public class Train {
	...
	public synchronized void doFeatureDumping() throws IOException {
	...
	public synchronized void doTraining() throws IOException {
		readTrainingSentences(examplesFilename);
		constructLabelVocab();
		extractFeatures();
	...
	public synchronized void readTrainingSentences(String filename) throws IOException {
		CoNLLReader conllR = new CoNLLReader();
		lSentences = conllR.readFile(filename);
		for (Sentence sent : lSentences)
			numTokens += sent.T();
	}

	public synchronized void constructLabelVocab() {
	...
	public synchronized void dumpFeatures() throws IOException {
	...
	public synchronized void extractFeatures() throws IOException {
	...
	public synchronized void readWarmStartModel() throws IOException {
		assert model.featureVocab.isLocked();
		Model mod = new Model();
		Model warmModel = mod.loadModelFromText(modelLoadFilename);
		mod.copyCoefsForIntersectingFeatures(warmModel, model);
	}

	public synchronized void optimizationLoop() {
	...
	private class GradientCalculator implements DiffFunction {
		...
		@Override
		public synchronized double valueAt(double[] flatCoefs) {
		...
		@Override
		public synchronized double[] derivativeAt(double[] flatCoefs) {
	...
	private synchronized void addL2regularizerGradient(double[] grad, double[] flatCoefs) {
	 ...
	 */
	 private synchronized double regularizerValue(double[] flatCoefs) {
		double l2_term = 0;
		for (int f=0; f < flatCoefs.length; f++) {
	...
	public class MyWeightsPrinter implements WeightsPrinter {
		...
		@Override
		public synchronized void printWeights() {
			double loglik = 0;
			for (ModelSentence s : mSentences) {
				loglik += model.computeLogLik(s);
			}
