package dz.pfe.storm.ressources.cmu.arktweetnlp.impl;
...
public class Model {
	...
	public synchronized int startMarker() {
		assert labelVocab.isLocked();
		int lastLabel = labelVocab.size() - 1;
		return lastLabel+1;
	}

	public synchronized void lockdownAfterFeatureExtraction() {
		labelVocab.lock();
		featureVocab.lock();
		allocateCoefs(labelVocab.size(), featureVocab.size());
	}

	public synchronized void allocateCoefs(int numLabels, int numObsFeats) {
	 ...
	 */
	public synchronized double[][] inferPosteriorGivenLabels(ModelSentence sentence) {
	 ...
	 **/
	public synchronized void greedyDecode(ModelSentence sentence, boolean storeConfidences) {
		int T = sentence.T;
	 ...
	 */
	public synchronized double[][] inferPosteriorForUnknownLabels(ModelSentence sentence) {
	 ...
	 */
	public synchronized void viterbiDecode(ModelSentence sentence) {
		int T = sentence.T;
		sentence.labels = new int[T];
	...
	private synchronized double[] getColumn(double[][] matrix, int col){
	...
	public synchronized void mbrDecode(ModelSentence sentence) {
	...
	public synchronized void computeLabelScores(int t, ModelSentence sentence, double[] labelScores) {
	...
	public synchronized void computeVitLabelScores(int t, int prior, ModelSentence sentence, double[] labelScores) {
		Arrays.fill(labelScores, 0);
		computeBiasScores(labelScores);
	...
	public synchronized void computeBiasScores(double[] labelScores) {
		for (int k=0; k < numLabels; k++) {
	...
	/** Adds into labelScores **/
	public synchronized void computeEdgeScores(int t, ModelSentence sentence, double[] labelScores) {
	...
	public synchronized void viterbiEdgeScores(int prior, ModelSentence sentence, double[] EdgeScores) {
		for (int k=0; k < numLabels; k++) {
	...
	/** Adds into labelScores **/
	public synchronized void computeObservedFeatureScores(int t, ModelSentence sentence, double[] labelScores) {
		for (int k=0; k < numLabels; k++) {
	...
	public synchronized double[] ThreewiseMultiply(double[] a, double[] b, double[] c) {
	...
	public synchronized void computeGradient(ModelSentence sentence, double[] grad) {
	...
	public synchronized double computeLogLik(ModelSentence s) {
	...
	public synchronized void setCoefsFromFlat(double[] flatCoefs) {
		for (int k=0; k<numLabels; k++) {
	...
	public synchronized double[] convertCoefsToFlat() {
	...
	public synchronized int flatIDsize() {
		int K = labelVocab.size();
		int J = featureVocab.size();
		// bias terms + edge features + observation features
		return K + (K+1)*K + J*K;
	}
	private synchronized int biasFeature_to_flatID(int label) {
		return label;
	}
	private synchronized int edgeFeature_to_flatID(int before, int current) {
		int K = labelVocab.size();
		return K + before*K + current;
	}
	private synchronized int observationFeature_to_flatID(int featID, int label) {
		int K = labelVocab.size();
	...
	public synchronized void saveModelAsText(String outputFilename) throws IOException {
		BasicFileIO bfIO = new BasicFileIO();
		BufferedWriter writer = bfIO.openFileToWriteUTF8(outputFilename);
	...
	public synchronized Model loadModelFromText(String filename) throws IOException {
		Model model = new Model();
		BasicFileIO bfIO = new BasicFileIO();
		BufferedReader reader = bfIO.openFileOrResource(filename);
		String line;
	 ...
	 */
	public synchronized void copyCoefsForIntersectingFeatures(Model sourceModel, Model destModel) {
