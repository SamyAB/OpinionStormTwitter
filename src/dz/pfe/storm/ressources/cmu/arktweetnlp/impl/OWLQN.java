...
public class OWLQN {
	...
	public Set<Integer> biasParameters = new THashSet<Integer>();
	//public ArrayList<Integer> biasParameters = new ArrayList<Integer>();

	interface TerminationCriterion {
		double getValue(OptimizerState state, StringBuilder out);
	}

	class RelativeMeanImprovementCriterion implements TerminationCriterion {
		int numItersToAvg;
		...
		public synchronized double getValue(OptimizerState state, StringBuilder out) {
	 ...
	 */
	public synchronized void setQuiet(boolean q) {
		quiet = q;
	}
	...
	*/
	 ...
	 */
	public synchronized double[] minimize(DiffFunction function, double[] initial, double l1weight, double tol, int m) {
	...
	private synchronized boolean arrayEquals(double [] x, double [] y){
	 ...
	 */
	public synchronized void setMaxIters(int maxIters) {
	 ...
	 */
	public synchronized int getMaxIters() {
		return maxIters;
	}

	/** Specify a callback for printing the weights after each iteration of optimization. */
	public synchronized void setWeightsPrinting(WeightsPrinter printer) {
		this.printer = printer;
	}

	private int numUnconstrainedWeights = -1;

	/** Sets {@link #constrained} */
	public synchronized void setConstrained(boolean constrained) {
		this.constrained = constrained;
		this.numUnconstrainedWeights = (constrained) ? 0 : -1;
	}
	 ...
	 */
	public synchronized void setConstrained(int numUnconstrainedWeights) {
		this.numUnconstrainedWeights = numUnconstrainedWeights;
		this.constrained = (numUnconstrainedWeights<0) ? false : true;
	}
	 ...
	 */
	protected synchronized double[] projectWeights(double[] c) {
	 ...
	 */
	public double[] project(double[] c) {
...
class OptimizerState {
	...
	private synchronized String arrayToString(double [] arr){
		String res = "";
	...
	private synchronized void printStateValues() {
		System.err.println("\nSLIST:");
