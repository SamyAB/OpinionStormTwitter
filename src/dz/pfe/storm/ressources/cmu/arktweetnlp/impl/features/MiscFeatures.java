...
public class MiscFeatures {
	...
	public class NextWord implements FeatureExtractorInterface{
		public void addFeatures(List<String> tokens, PositionFeaturePairs pairs) {
			...
			ArrayList<String> normtoks = fea.normalize(tokens);
	...
	public class SimpleOrthFeatures implements FeatureExtractorInterface {
		...
		Pattern emoticon = Pattern.compile(to.emoticon);
		public void addFeatures(List<String> tokens, PositionFeaturePairs pairs) {
			for (int t=0; t < tokens.size(); t++) {
				String tok = tokens.get(t);
				...
				if (tok.contains("-")){
					...
					FeatureUtil fea = new FeatureUtil();
					String[] splithyph = fea.normalize(tok).split("-", 2);
	...
	public class URLFeatures implements FeatureExtractorInterface {
		Twokenize to = new Twokenize();
		Pattern validURL = Pattern.compile(to.url);
		Pattern validEmail = Pattern.compile(to.Email);
		public void addFeatures(List<String> tokens, PositionFeaturePairs pairs) {
			for (int t=0; t < tokens.size(); t++) {
				String tok = tokens.get(t);
	...
	public class WordformFeatures implements FeatureExtractorInterface {
		public void addFeatures(List<String> tokens, PositionFeaturePairs pairs) {
			for (int t=0; t < tokens.size(); t++) {
				...
				FeatureUtil fea = new FeatureUtil();
				pairs.add(t, "Lower|" + fea.normalize(normalizedtok));
	...
	public class NgramPrefix implements FeatureExtractorInterface {
		...
		public void addFeatures(List<String> tokens, PositionFeaturePairs pairs) {
			for (int t=0; t < tokens.size(); t++) {
				FeatureUtil fea = new FeatureUtil();
				String tok = fea.normalize(tokens.get(t).replaceAll("[‘’´`]", "'").replaceAll("[“”]", "\""));
				int l=tok.length();
	...
	public NgramSuffix getNewNgramSuffix(int i){
		return (new NgramSuffix(i));
	}

	public NgramPrefix getNewNgramPrefix(int i){
		return (new NgramPrefix(i));
	}

	public PrevWord getNewPrevWord(){
		return (new PrevWord());
	}
	public NextWord getNewNextWord(){
		return (new NextWord());
	}
	public WordformFeatures getNewWordformFeatures(){
		return(new WordformFeatures());
	}

	public CapitalizationFeatures getNewCapitalizationFeatures(){
		return(new CapitalizationFeatures());
	}

	public SimpleOrthFeatures getNewSimpleOrthFeatures(){
		return(new SimpleOrthFeatures());
	}

	public PrevNext getNewPrevNext(){
		return(new PrevNext());
	}
